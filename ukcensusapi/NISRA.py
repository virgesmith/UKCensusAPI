...
class NISRA:
    """
  Scrapes and reformats NI 2011 census data from NISRA website
  """
    # static constants
    ...
    def __get_metadata_impl(self, table, resolution):
        ...
        if len(commas) > 1 and min_categories > 0:
            print("WARNING: it appears that {} is multivariate and some category descriptions contain a comma. ".format(
                table) + \
        ...
        if min_categories > 0:
            raw_meta = pd.concat([raw_meta["ColumnVariableCode"],
                                  raw_meta["ColumnVariableDescription"].str.split(", ", n=min_categories, expand=True)],
                                 axis=1)
        else:
...
def _ni_resolution(resolution):
    """
  Maps E&W statistical geography codes to their closest NI equivalents
  """
