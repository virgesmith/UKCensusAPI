...
class Nomisweb:
  ...
  """
    ...
    def get_lad_codes(self, la_names):
        """Convert local authority name(s) to nomisweb codes.
    Args:
    ...
    """
        ...
        # python dicts have non-deterministic order, see
    ...
    """
        ...
        for rawfield in rawfields:
            ...
            # ignore when too many categories (i.e. geographical ones)
    ...
    # loads metadata from cached json if available, otherwise downloads from nomisweb.
    ...
    def __cache_lad_codes(self):
        ...
        if not os.path.isfile(str(filename)):
            ...
            data = self.__fetch_json("api/v01/dataset/NM_144_1/geography/"
                                     + str(Nomisweb.GeoCodeLookup["EnglandWales"]) + Nomisweb.GeoCodeLookup[
                                         "LAD"] + ".def.sdmx.json?", {})
            if data == {}:
                return []
